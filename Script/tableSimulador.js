...
function fillTable(header,movements){
    var tabla = "";

    if(header['error'] !== true){
        tabla +="<p class='text-center'>EL MES CONSULTADO POSEE ENCABEZADO POR FAVOR CONSULTAR POR MES O POR RANGO</p>"
        ...
        tabla+= "<table class='table table-striped table-sm table-bordered' style='width:60%;display:inline-table'>";
            ...
            tabla+="</thead>";
            tabla+="<tbody>";
            var account = header['data']['CUENTA'];
            var name = header['data']['NOMBRE'];
            var direccion = header['data']['DIRECCION'];
            var moneda = header['data']['MONEDA'];
                    tabla+="<tr class='table-body'>";
                    tabla+="<td class='text-center'>"+account+"</td>";
                    tabla+="<td class='text-left'>"+name+"</td>";
                    tabla+="<td class='text-right'><input type='number' class='form-control form-control-sm text-center form-search' id='saldoInicial' name='saldoInicial' autocomplete='off'></input></td>";
                tabla+="</tr>";
        tabla+="</tbody>";
        tabla+="</table>";
        tabla+="<p id=imprimir class='printerMonth'></p>"
        $("#tableDinamic").append(tabla);

        $("#saldoInicial").keypress(function(e) {
                        var code = (e.keyCode ? e.keyCode : e.which);
                        if(code==13){
                            balanceIncial = $(this).val();
                            CrearTablaMovements(movements,balanceIncial,name,direccion,moneda);
  ...
  function CrearTablaMovements(movements,balanceIncial,name,direccion,moneda){
    var tabla = "";
    let count = 0;
    let sumaDebito = 0;
    let sumaCredito = 0;
  ...
  tabla+="</table>";
    ...
    $("#tableDinamic").append(tabla);
    CrearTablaResume(subtotal,count,sumaDebito,sumaCredito,sum,balanceIncial,name,direccion,moneda);
...
//   }
   ...
   function CrearTablaResume(subtotal,count,sumaDebito,sumaCredito,sum,balanceIncial,name,direccion,moneda){
    var tabla = "";
    ...
    let saldoFinal = formatMoney(parseFloat(sum).toFixed(2));
    ...
    var year = $('#Year').val();
      ...
      tabla+="<table class='table table-striped table-sm table-bordered' style='float:right;width:60%;'>";
          ...
          tabla+="<tbody>";
              ...
              tabla+="<td class='text-right'>"+saldoFinal+"</td>";        
              tabla+="</tr>";
                     
          tabla+="</tbody>";
      tabla+="</table>";
    $("#tableDinamic").append(tabla);
    var button = "<a download href='../Page/report.php?account="+cuenta+"&nombre="+name+"&direccion="+direccion+"&saldofinal="+saldoFinal+"&saldoinicial="+balanceIncial+"&saldopromedio="+promedio+"&totalcredito="+sumadeCreditos+"&totaldebitos="+sumadeDebitos+"&mes="+month+"&year="+year+"&moneda="+moneda+"' class='btn  btn-print print'><i class='fas fa-print'></i></a>";
    $("#imprimir").append(button);
  }
